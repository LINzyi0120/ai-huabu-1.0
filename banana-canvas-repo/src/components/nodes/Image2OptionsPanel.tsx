...
export function Image2OptionsPanel({ value, hasReferenceImages, onChange }: Image2OptionsPanelProps) {
  ...
  const parameterTips = [
    ...
    '画面比例选择会覆盖全局 aspect_ratio 设置，直接指定 size 参数发送给 API。',
    ...getImage2RelayParameterTips(),
  ];
    ...
    <div className="space-y-4 rounded-xl p-3" style={{ background: 'rgba(242,193,78,0.04)', border: '1px solid rgba(242,193,78,0.12)' }}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <label className="text-[11px] font-medium" style={labelStyle}>画面比例 / 分辨率</label>
          <select
            ...
            onChange={(event) => {
              ...
              if (v === 'auto') {
                ...
                setOption({ size: v as Image2Size });
              }
            }}
            className="nowheel w-full rounded-lg p-2 text-xs outline-none"
            style={selectStyle}
          >
            <option value="auto">自动（跟随比例）</option>
            <option value="1024x1024">1024×1024（1:1 方形）</option>
            <option value="1536x1024">1536×1024（3:2 横版）</option>
            <option value="1024x1536">1024×1536（2:3 竖版）</option>
            <option value="1792x1024">1792×1024（16:9 宽屏）</option>
            <option value="1024x1792">1024×1792（9:16 竖屏）</option>
          </select>
        </div>
