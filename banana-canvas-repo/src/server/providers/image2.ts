...
export function toImage2Size(aspectRatio?: string, imageSize?: string, explicitSize?: string) {
  // User explicitly picked a size in the options panel — use it directly
  if (explicitSize) return explicitSize;
  // Fall back to aspect-ratio / imageSize mapping
  ...
  const createRequestInit = (): RequestInit => {
    if (image2Config.endpointType === 'images' && images.length > 0) {
      const request = buildImage2MultipartRequest({
        ...
        size: toImage2Size(aspectRatio, imageSize, normalizedImage2Options.size),
